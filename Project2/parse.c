/***************************************************************/
/*                                                             */
/*   MIPS-32 Instruction Level Simulator                       */
/*                                                             */
/*   CS311 KAIST                                               */
/*   parse.c                                                   */
/*                                                             */
/***************************************************************/

#include <stdio.h>

#include "util.h"
#include "parse.h"

int text_size;
int data_size;

instruction parsing_instr(const char *buffer, const int index)
{
    instruction instr;
	/** Implement this function */
	int i;
	
	uint32_t value =0;
	for (i=0; i<=31; i++) value = value*2 + (uint32_t)buffer[i]-48;
	instr.value = value;
	
	short opcode = 0;	
	for (i=0; i<=5; i++) opcode = opcode*2 + (short)buffer[i]-48;
	instr.opcode = opcode;
	
	
	if (opcode ==2 || opcode == 3){
		uint32_t target=0;
		for (i=6; i<=31; i++) target = target*2 + (uint32_t)buffer[i]-48;
		instr.r_t.target = target;
	}
	else{
		unsigned char  rs=0;
		unsigned char rt=0;
		for (i=6; i<=10; i++) rs = rs*2 + (unsigned char)buffer[i]-48;
		for (i=11; i<=15; i++) rt = rt*2 + (unsigned char)buffer[i]-48;
		instr.r_t.r_i.rs=rs;
		instr.r_t.r_i.rt=rt;
		if (opcode==0){
			short f_code=0;
			unsigned char rd=0;
			unsigned char shamt=0;
			for (i=16; i<=20; i++) rd = rd*2 + (unsigned char)buffer[i]-48; 
			for (i=21; i<=25; i++) shamt = shamt*2 + (unsigned char)buffer[i]-48;
			for (i=26; i<=31; i++) f_code = f_code*2 + (short)buffer[i]-48;
			instr.func_code = f_code;
			instr.r_t.r_i.r_i.r.rd = rd;
			instr.r_t.r_i.r_i.r.shamt = shamt;
		}
		else{
			short imm=0;
			for (i=16; i<=31; i++) imm = imm*2 + (short)buffer[i]-48;
			instr.r_t.r_i.r_i.imm = imm;
		}
	}
	
    return instr;
}

void parsing_data(const char *buffer, const int index)
{
	/** Implement this function */
	int data = 0;
	int i;
	for (i=0; i<=31; i++) data =  data*2 + (uint32_t) buffer[i]-48;
	uint32_t data_address = MEM_DATA_START + index;
	mem_write_32(data_address, data);
}

void print_parse_result()
{
    int i;
    printf("Instruction Information\n");

    for(i = 0; i < text_size/4; i++)
    {
	printf("INST_INFO[%d].value : %x\n",i, INST_INFO[i].value);
	printf("INST_INFO[%d].opcode : %d\n",i, INST_INFO[i].opcode);

	switch(INST_INFO[i].opcode)
	{
	    //Type I
	    case 0x9:		//(0x001001)ADDIU
	    case 0xc:		//(0x001100)ANDI
	    case 0xf:		//(0x001111)LUI	
	    case 0xd:		//(0x001101)ORI
	    case 0xb:		//(0x001011)SLTIU
	    case 0x23:		//(0x100011)LW	
	    case 0x2b:		//(0x101011)SW
	    case 0x4:		//(0x000100)BEQ
	    case 0x5:		//(0x000101)BNE
		printf("INST_INFO[%d].rs : %d\n",i, INST_INFO[i].r_t.r_i.rs);
		printf("INST_INFO[%d].rt : %d\n",i, INST_INFO[i].r_t.r_i.rt);
		printf("INST_INFO[%d].imm : %d\n",i, INST_INFO[i].r_t.r_i.r_i.imm);
		break;

    	    //TYPE R
	    case 0x0:		//(0x000000)ADDU, AND, NOR, OR, SLTU, SLL, SRL, SUBU  if JR
		printf("INST_INFO[%d].func_code : %d\n",i, INST_INFO[i].func_code);
		printf("INST_INFO[%d].rs : %d\n",i, INST_INFO[i].r_t.r_i.rs);
		printf("INST_INFO[%d].rt : %d\n",i, INST_INFO[i].r_t.r_i.rt);
		printf("INST_INFO[%d].rd : %d\n",i, INST_INFO[i].r_t.r_i.r_i.r.rd);
		printf("INST_INFO[%d].shamt : %d\n",i, INST_INFO[i].r_t.r_i.r_i.r.shamt);
		break;

    	    //TYPE J
	    case 0x2:		//(0x000010)J
	    case 0x3:		//(0x000011)JAL
		printf("INST_INFO[%d].target : %d\n",i, INST_INFO[i].r_t.target);
		break;

	    default:
		printf("Not available instruction\n");
		assert(0);
	}
    }

    printf("Memory Dump - Text Segment\n");
    for(i = 0; i < text_size; i+=4)
	printf("text_seg[%d] : %x\n", i, mem_read_32(MEM_TEXT_START + i));
    for(i = 0; i < data_size; i+=4)
	printf("data_seg[%d] : %x\n", i, mem_read_32(MEM_DATA_START + i));
    printf("Current PC: %x\n", CURRENT_STATE.PC);
}

...
void  MEM_Stage(){
	...
	if (PRE_STATE.PIPE_STALL[MEM_STAGE] == FALSE){
		...
		if (PRE_STATE.EX_MEM_ins.opcode == 0x2b){
			//cache use: mem_write_32(uint32_t address, uint32_t value){
			//mem_write_32(PRE_STATE.EX_MEM_ALU_OUT, wdata);
			cache_write_32(PRE_STATE.EX_MEM_ALU_OUT, wdata);
		}
		...
		if (PRE_STATE.EX_MEM_ins.opcode == 0x23){
			...
			CURRENT_STATE.MEM_WB_MEM_OUT = cache_read_32(PRE_STATE.EX_MEM_ALU_OUT);
